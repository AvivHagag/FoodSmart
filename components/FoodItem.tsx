...
const FoodItem: React.FC<FoodItemProps> = ({
  ...
  const [value, setValue] = useState(unit === 'gram' ? (avg_gram || 0) : initialCount);
    ...
    <View className="bg-white rounded-xl p-4 mb-4 shadow-lg shadow-black/10">
      ...
      <View className="flex-row flex-wrap gap-4">
        <View className="flex-1 min-w-[45%] bg-indigo-50 p-3 rounded-lg">
          <Text className="text-sm text-indigo-600 font-medium mb-1">Calories</Text>
        ...
        <View className="flex-1 min-w-[45%] bg-green-50 p-3 rounded-lg">
          <Text className="text-sm text-green-600 font-medium mb-1">Protein</Text>
          <Text className="text-xl font-bold text-gray-900">
